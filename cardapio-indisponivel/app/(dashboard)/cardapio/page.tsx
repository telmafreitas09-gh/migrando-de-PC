...
export default function CardapioPage() {
  const { pratos, cardapioDia, user, settings } = useApp()
      ...
      <div className="p-6 lg:p-8 max-w-7xl mx-auto">
          ...
          <div className="flex flex-wrap items-center gap-2">
            {isAdmin && cardapioPratos.length > 0 && (
              <>
                <Link
                  href="/arte?gerado=1"
                  className="inline-flex h-7 items-center gap-2 rounded-[min(var(--radius-md),12px)] border border-border bg-background px-2.5 text-[0.8rem] font-medium transition-colors hover:bg-muted"
                >
                  <Sparkles size={15} />
                  Gerar arte
                </Link>
        ...
        {isGuest && (
          <div className="no-print mb-6 flex items-start gap-3 bg-muted border border-border rounded-2xl px-4 py-3 text-sm text-muted-foreground">
            ...
            <span>
              Bem-vindo ao <strong className="text-foreground">{settings.nome}</strong>! Escolha seus
              pratos e adicione ao carrinho para fazer o pedido.
            </span>
          </div>
        )}
        ...
        {isAdmin && (
          <div className="no-print mb-6 flex items-start gap-3 bg-primary/8 border border-primary/20 rounded-2xl px-4 py-3 text-sm text-primary">
            ...
            <span>
              Para alterar os pratos do cardápio, vá em <strong>Pratos</strong> e use o checkbox{' '}
              <strong>Cardápio do dia</strong>. Depois, gere a arte em <strong>Arte</strong>.
            </span>
          </div>
        )}
        ...
        {cardapioPratos.length === 0 ? (
          <div className="text-center py-24 text-muted-foreground">
            ...
            {isAdmin && (
              <p className="mt-2 text-sm">
                Vá em <strong>Pratos</strong> e marque o checkbox &quot;Cardápio do dia&quot; nos
                pratos desejados.
              </p>
            )}
          </div>
          ...
          <>
            <div id="cardapio-print" className="mb-8 flex justify-center">
              <MenuArtPreview
                            ...
                            {inCart ? (
                              <div className="flex items-center gap-2">
                                <button
                                  type="button"
                                  onClick={() =>
                                    updateQtyForPrato(prato.id, inCart.quantidade - 1, pratoExtras)
                                  }
                              ...
                              <Button
                                size="sm"
                                onClick={() => handleAddToCart(prato.id)}
                                className="gap-1"
                              >
                                <Plus size={13} />
                                Adicionar
