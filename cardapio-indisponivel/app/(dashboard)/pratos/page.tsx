...
import { Plus, Search, Pencil, Trash2, ToggleLeft, ToggleRight, CalendarDays, Sparkles, Image } from 'lucide-react'
...
export default function PratosPage() {
  ...
  const { pratos, addPrato, updatePrato, deletePrato, cardapioDia, toggleCardapioDia, user, settings } = useApp()
  ...
  function handleGerarArte() {
    if (cardapioCount === 0) {
      toast.error('Selecione ao menos um prato no cardápio do dia.')
      return
    }
    router.push('/arte?gerado=1')
  }
    ...
    <div className="p-6 lg:p-8 max-w-7xl mx-auto">
      ...
      {isAdmin && (
        <div className="mb-6 flex items-start gap-3 bg-primary/8 border border-primary/20 rounded-2xl px-4 py-3 text-sm text-primary">
          ...
          <span>
            Use o checkbox <strong>Cardápio do dia</strong> em cada prato para compor o menu de hoje.
            Quando terminar a seleção, clique em <strong>Gerar Arte</strong> para abrir a página de
            divulgação com a arte pronta.
          </span>
        </div>
      )}

      {isAdmin && cardapioCount > 0 && (
        <div className="mb-6 rounded-2xl border border-border bg-card p-4 lg:p-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center">
            <div className="flex-1">
              <div className="mb-2 flex items-center gap-2">
                <Image size={16} className="text-primary" />
                <h2 className="text-sm font-semibold">Prévia do cardápio gerado</h2>
              </div>
              <p className="text-xs text-muted-foreground">
                {cardapioCount} prato(s) selecionado(s) · a arte será gerada com base nesta seleção
              </p>
            </div>
            <div className="flex justify-center lg:justify-end">
              <div className="scale-[0.55] origin-center lg:origin-right -my-8 lg:-my-10">
                <MenuArtPreview
                  pratos={cardapioPratos.slice(0, 4)}
                  format="feed"
                  settings={settings}
                />
              </div>
            </div>
            <Button onClick={handleGerarArte} className="gap-2 lg:flex-shrink-0">
              <Sparkles size={16} />
              Gerar Arte
            </Button>
          </div>
      ...
      {isAdmin && cardapioCount > 0 && (
        <div className="fixed bottom-6 left-1/2 z-30 flex -translate-x-1/2 items-center gap-3 rounded-2xl border border-border bg-card/95 px-4 py-3 shadow-xl backdrop-blur-sm lg:left-[calc(50%+7rem)]">
          <span className="hidden text-sm text-muted-foreground sm:inline">
            {cardapioCount} no cardápio de hoje
          </span>
          <Button onClick={handleGerarArte} size="sm" className="gap-2">
            <Sparkles size={15} />
            Gerar Arte
          </Button>
        </div>
